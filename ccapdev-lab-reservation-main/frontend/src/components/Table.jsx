...
export default function Table({ bordercolor, bgcolor }) {
    ...
    useEffect(() => {
        fetchReservations();
    }, [sortField, order, page]);

    const fetchReservations = async () => {
        try {
            const response = await axios.get(`/api/reservations?page=${page}&sort=${sortField}&order=${order}`);
    ...
    return (
        <>
            <div className='my-3 flex flex-row'>
                ...
                <button onClick={() => setPage(page < totalPages ? page + 1 : totalPages)}>
                    <IconChevronRight stroke={2} />
                </button>
            </div>
            <table className={`w-full table-auto my-3 rounded-sm bg-white border-solid border-2 ${bordercolor} text-left border-separate border-spacing-0`}>
                <thead className='sticky top-0 shadow'>
                    <tr className={`${bgcolor}`}>
                        <th onClick={() => handleSortClick('userEmail')} className='cursor-pointer'>User {sortField === 'userEmail' && (order === 'asc' ? <IconSortAscending /> : <IconSortDescending />)}</th>
                        <th onClick={() => handleSortClick('labName')} className='cursor-pointer'>Room {sortField === 'labName' && (order === 'asc' ? <IconSortAscending /> : <IconSortDescending />)}</th>
                        <th>Seat</th>
                        <th onClick={() => handleSortClick('startTime')} className='cursor-pointer'>Time {sortField === 'startTime' && (order === 'asc' ? <IconSortAscending /> : <IconSortDescending />)}</th>
                    </tr>
                </thead>
                <tbody>
                    {reservations.map(res => (
                        <tr key={res._id} className='odd:bg-white even:bg-fieldgray hover:bg-bgblue' onClick={() => navigate(`/reservation/${res._id}`)}>
                            <td>{res.isAnonymous ? 'Anonymous' : res.userEmail}</td>
                            <td>{res.labName}</td>
